...
4. Deploy to Kubernetes
   ...
   kubectl create namespace electronics-store-namespace
   kubectl apply -f deployment-electronics.yaml -n electronics-store-namespace
   kubectl apply -f service-electronics.yaml -n electronics-store-namespace
...
If you need to apply the HPA configuration, you can do so with the following steps:
  kubectl apply -f hpa-electronics.yaml -n electronics-store-namespace
Testing HPA
To test the functionality of the HPA:

1. Check HPA Status:
  kubectl get hpa -n electronics-store-namespace
...
3. Monitor Pods: After generating load, monitor the number of pods:
  kubectl get pods -n electronics-store-namespace
4. Check Logs: You can check the logs to see the scaling events:
   kubectl logs -l app=electronics-store -n electronics-store-namespace
